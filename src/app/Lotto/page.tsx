...
export default function Lotto() {
    ...
    const HandleClick = () => {
        ...
        const bounus : number[] = num.splice(-1);
        
        num.sort((a,b) => a-b);

                    // 20250805-3 배열 타입 명시
                    // number or string이 들어올 수 있는 배열이다!
        const lotto : (number | string)[] = [...num, '+', ...bounus];

        // 20250805-4 다른변수를 써라! => tmTag
                                                //typeof 문자열 이면, 스팬태그 날려라!
        const tmTag : ReactNode[] = lotto.map(item => typeof item == 'string' ? <span className = "font-bold text-3xl" key={`n${item}`}>{item}</span>
